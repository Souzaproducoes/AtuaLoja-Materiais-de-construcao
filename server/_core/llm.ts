...
type LlmProviderConfig = Pick<typeof ENV, "nvidiaApiKey" | "nvidiaBaseUrl" | "nvidiaModel" | "groqApiKey" | "groqBaseUrl" | "groqModel" | "isProduction">;

export function getConfiguredLlmProviders(config: LlmProviderConfig = ENV): LlmProvider[] {
...
export async function listLLMModels(): Promise<ModelsResponse> {
  ...
  const provider = getConfiguredLlmProviders()[0];
  if (!provider) throw new Error("Configure NVIDIA_API_KEY ou GROQ_API_KEY no .env para listar modelos.");
  const response = await fetchWithBackoff(`${provider.baseUrl.replace(/\/$/, "")}/models`, {
